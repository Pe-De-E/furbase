  ...
  return (
    <form action={saveChecklistItem} className="flex flex-col gap-6 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 p-6 flex flex-col gap-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700">{t('fieldTextDe')}</label>
            <input name="textDe" required defaultValue={item?.textDe} className={inputCls} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700">{t('fieldTextEn')}</label>
            <input name="textEn" required defaultValue={item?.textEn} className={inputCls} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700">{t('fieldDescDe')}</label>
            <textarea name="descriptionDe" rows={2} defaultValue={item?.descriptionDe ?? ''} className={inputCls} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700">{t('fieldDescEn')}</label>
            <textarea name="descriptionEn" rows={2} defaultValue={item?.descriptionEn ?? ''} className={inputCls} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700">{t('fieldOrder')}</label>
            <input
              name="sortOrder"
              type="number"
      ...
      <div className="flex items-center gap-3">
        ...
        <Link
          href="/admin/adoption"
          className="px-5 py-2.5 text-sm text-zinc-500 hover:text-zinc-900 transition-colors"
        >
          {t('cancel')}
        </Link>
      </div>
