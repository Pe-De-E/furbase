...
const HOURS_MAP: Record<string, number> = {
  '0-2': 2, '2-4': 4, '4-6': 6, '6+': 8,
}

export async function saveMatcherProfile(userId: string, profile: MatcherProfile) {
  const values = {
    userId,
    ...
    preferredSpecies: profile.preferredSpecies ? [profile.preferredSpecies] : [],
