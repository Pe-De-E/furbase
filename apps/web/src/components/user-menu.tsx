...
import { LayoutDashboard, LogOut, User, ClipboardList, CalendarDays, HelpCircle, MessageSquare } from 'lucide-react'
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-lg py-2 z-50">
          ...
          <Link
            ...
            className="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
          >
            <MessageSquare className="inline-block mr-2 w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500" />
            {t('feedback')}
          </Link>
          ...
          {isAdmin && (
            <div className="border-t border-zinc-100 dark:border-zinc-800 mt-1 pt-1">
              ...
              <Link
                ...
                className="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
              >
                <HelpCircle className="inline-block mr-2 w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500" />
                {t('faq')}
              </Link>
            </div>
          )}
